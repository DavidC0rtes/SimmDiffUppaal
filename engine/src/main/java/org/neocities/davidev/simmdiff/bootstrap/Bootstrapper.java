package org.neocities.davidev.simmdiff.bootstrap;

import org.neocities.davidev.simmdiff.core.Runner;
import org.neocities.davidev.simmdiff.grammars.diff.DiffLexer;
import org.neocities.davidev.simmdiff.grammars.diff.DiffParser;
import org.neocities.davidev.simmdiff.grammars.diff.ExtendedDiffListener;
import org.neocities.davidev.simmdiff.grammars.uppaal.NTAListener;
...
public class Bootstrapper {
    ...
    public Bootstrapper(File mutant, String diffStr) throws IOException {
        ...
        parse(mutantFile, diffListener.getLineRanges());
        runner = new Runner(NTAlistener.getNameToTemplate(), NTAlistener.getGlobalClocks(), NTAlistener.getGlobalDeclarations());
        runner.start();
    }

    public Bootstrapper(File model) throws IOException {
        mutantFile = model;
        parse(mutantFile, new ArrayList<>());
        runner = new Runner(NTAlistener.getNameToTemplate(), NTAlistener.getGlobalClocks(), NTAlistener.getGlobalDeclarations());
        runner.start();
    }
